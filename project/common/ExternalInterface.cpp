...
void WebpInitTables()
{
	...
	for (i = RGB_RANGE_MIN; i < RGB_RANGE_MAX; ++i)
	{
		...
		kClip[i - RGB_RANGE_MIN] = (j < 0) ? 0 : (j > 255) ? 255
														   : j;
	}
...
extern "C"
{
	...
	{
		if (!val_is_kind(_value, gCodecContextKind))
			val_throw(alloc_string("Not a codec context"));
		return (HxWebmContext *)val_get_handle(_value, gCodecContextKind);
	}

	io_struct *_get_io_struct_from_value(value _value)
	{
		if (!val_is_kind(_value, gIoStructKind))
			val_throw(alloc_string("Not a io_struct"));
	...
	namespace mkvparser
	{
		...
		{
			if (!val_is_kind(_value, kind_MkvProcessor))
				val_throw(alloc_string("Not a MkvProcessor"));
